...
В [предыдущей главе](01_appproperties__.md) мы разобрались, как адаптер собирает настройки (`AppProperties`), чтобы знать, куда и как отправлять результаты тестов. Теперь все готово для подключения к Test IT. Но как сам адаптер поймет, *какой именно* тест вы запускаете и какую дополнительную информацию о нем нужно передать?
