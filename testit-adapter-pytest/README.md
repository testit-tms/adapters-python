...
| Test run links (**It's optional**). JSON array of objects with required `url` and optional `title`, `description`, `type`. Applied at create/start (not only at the end). Link types: `Related`, `BlockedBy`, `Defect`, `Issue`, `Requirement`, `Repository`                                                                                                                                                                    | testRunLinks                      | TMS_TEST_RUN_LINKS                         | tmsTestRunLinks                      |
